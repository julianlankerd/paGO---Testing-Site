...
JDEBUG ? $_PROFILER->mark('afterLoad') : null;
require_once JPATH_ADMINISTRATOR . DS . 'modules' . DS . 'mod_pago_dashboard_bestselling' . DS . 'helper.php';
...
{
	modPagoBestsellHelper::bestSellingAvg($selected);
}
elseif ($selected == 'customdate')
{
	$sale_start_date = $get['sale_start_date'];
	$sale_end_date = $get['sale_end_date'];
	modPagoBestsellHelper::bestSellingAvg($selected, $sale_start_date, $sale_end_date);
}
