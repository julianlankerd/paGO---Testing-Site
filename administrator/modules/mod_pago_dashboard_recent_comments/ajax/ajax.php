...
JDEBUG ? $_PROFILER->mark('afterLoad') : null;
require_once JPATH_ADMINISTRATOR . DS . 'modules' . DS . 'mod_pago_dashboard_recent_comments' . DS . 'helper.php';
//require_once JPATH_ADMINISTRATOR . '/modules/mod_pago_dashboard_recent_comments/language/en-GB.mod_pago_dashboard_recent_comments.ini';
...
{
	modPagoRecentCommentsHelper::latestCommentsAvg($selected);
}
elseif ($selected == 'customdate')
{
	$sale_start_date = $get['sale_start_date'];
	$sale_end_date = $get['sale_end_date'];
	modPagoRecentCommentsHelper::latestCommentsAvg($selected, $sale_start_date, $sale_end_date);
}
