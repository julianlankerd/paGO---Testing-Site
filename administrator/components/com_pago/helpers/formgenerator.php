 ...
 */
     ...
     */
    public function display($label=false, $id=false) {
        if ($id){
        ...
        if ((isset($this->error)) || ($this->getConfig("showAfterSuccess")) || (!isset($this->input[$this->getConfig("submitField")]))) {
            ...
            if (isset($this->protectionCode)) {
             ...
             if ($id){
                 $this->form.= "<input type='submit' id='submit_$id' name='$id' value='$label' ></input>\n";
             }
     ...
     */
    public function addField($type, $id,$label,$mandatory, $init_value=false, $aux=false, $attribs=false) {
        ...
        if (in_array($type, $allowed)) {
            $this->fields[$id]=Array("type" =>$type,"label" =>$label,"mandatory" =>$mandatory, "init_value" =>$init_value, "aux" =>$aux, "attribs" =>$attribs);
        }
        if($type=="file") {
    ...
    * @param $label label of item
    ...
    protected function createFields() {
		...
		if ($this->fields) {
            $props=Array("type", "label", "content", "acceskey", "mandatory", "init_value", "aux", "validator", "args", "grouped", "attribs");
            foreach ($this->fields as $id =>$field) {
                ...
                if ($type) {
                    if (($type=="auxtext")||($type=="auxitem")) {
                        ...
                        if (($label)&&($type!="hidden")) {
                            $this->form.= "<label class='".$id."' for='".$id."'";
                            if($acceskey) {
                        ...
                        switch ($type) {
                            ...
                            case "radio": $this->radiobuttons($id, $mandatory, $init_value, $aux);
                                break;
                            case "select": $this->selectfield($id, $mandatory, $init_value, $aux, $attribs);
                                break;
    ...
    protected function passwordfield($id,$mandatory, $init_value, $aux) {
        $this->form.= "<input type='password' id='".$id."' class='".$id."' name='". $id. "' value='";
        if (isset($this->input[$id])) {
        ...
        $this->form.= "' ". $aux . " />";
        if((!isset($this->fields[$id]["grouped"]))||($this->fields[$id]["grouped"]==3)){
    ...
    protected function hiddenfield($id,$mandatory, $init_value, $aux) {
        $this->form.= "<input type='hidden' id='".$id."' class='".$id."' name='". $id. "' value='";
        if (isset($this->input[$id])) {
            $this->form.= $this->input[$id];
        } elseif($init_value) {
            $this->form.=$init_value;
    ...
    protected function textfield($id,$mandatory, $init_value, $aux) {
        $this->form.= "<input type='text' id='".$id."' class='".$id."' name='". $id. "' value='";
        if (isset($this->input[$id])) {
            $this->form.= $this->input[$id];
        } elseif($init_value) {
            $this->form.=$init_value;
    ...
    protected function filefield($id,$mandatory, $aux) {
        $this->form.= "<input type='file' id='".$id."' class='".$id."' name='". $id. "' ". $aux . " />";
        if((!isset($this->fields[$id]["grouped"]))||($this->fields[$id]["grouped"]==3)){
             $this->form.=$this->getConfig("linebreaks");
        }
         $this->form.= "\n";
    ...
    protected function textarea($id,$mandatory, $init_value, $aux) {
        $this->form.= "<textarea id='".$id."' class='".$id."' name='". $id. "' ". $aux . ">";
        if (isset($this->input[$id])) {
            $this->form.= $this->input[$id];
        } elseif($init_value) {
            $this->form.=$init_value;
     ...
     * @param $options options array (name => value)
     */
    protected function selectfield($id,$mandatory, $init_value, &$options, &$attribs=false) {

        $this->form.= "<select id='".$id."' class='".$id."' name='". $id. "' >";
        foreach($options as $key => $value) {
            $this->form.= "<option ".$attribs[$key]."value='" . $value. "' ";
            if ( ((isset($this->input[$id]))&&($this->input[$id]==$value)) || $init_value==$value) {
    ...
    protected function checkbox($id,$mandatory, $init_value, $options) {
        if (is_array($options)) {
            ...
            $i = 0;
            foreach ($options as $key => $value) {
                $this->form.= "<label class='sublabel' for='" . $id."_" . $value. "'><input type='checkbox' class='".$id."' id='".$id."_" . $value ."' name='". $id."_" . $value. "' ";
            ...
            $this->form.= "<label class='sublabel' for='" . $id. "'><input type='checkbox' class='".$id."' id='".$id."' name='". $id. "' ";
    ...
    protected function createButton($type, $id, $value,$aux){
        ...
        if (in_array($type, $allowed)){
            ...
            $this->form.="<input type='". $type . "' id='".$id . "' class='".$id."' name='".$id . "' value='".$value . "' ". $aux. "></input>";
            $this->form.=$this->getConfig("linebreaks");
        }
    }
    /**
    ...
    protected function writeErrors() {
        if (($this->error)&&($this->config["showErrors"]["value"])) {
            $this->errorBox = "<div class='errorbox'><h4>". $this->config["errorTitle"]["value"]."</h4>\n";
