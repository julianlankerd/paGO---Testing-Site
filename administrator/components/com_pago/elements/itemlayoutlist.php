...
class JFormFieldItemlayoutlist extends JFormField
{
	protected $type = 'itemlayoutlist';
	...
	function getInput()
	{
		...
		foreach ($componentFolders as $key => $componentFolder) {
			$customLayoutPath    = JPATH_SITE . DS . 'components' . DS . 'com_pago' . DS . 'templates' . DS . $componentFolder . DS . 'item' . DS. 'default.php';
		...
		if($joomla_theme){
			...
			$templatesFolders = JFolder::folders( $templatesFolderPath );
				foreach ($templatesFolders as $key => $templatesFolder) {
					$templatesLayoutPath    = JPATH_SITE . DS . 'templates' . DS . $joomla_theme . DS . 'html' . DS . 'com_pago' . DS . $templatesFolder . DS . 'item' . DS .'default.php';
