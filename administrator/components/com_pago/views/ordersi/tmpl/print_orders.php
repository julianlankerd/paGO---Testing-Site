...
$cidStr 		= $jinput->get('cidStr', 0, 'string');
$cidStr = urldecode($cidStr);
$ordArray = explode(",", $cidStr);
$doc = JFactory::getDocument();
//$doc->addStyleSheet(JURI::root() . 'components/com_pago/templates/default/css/pago.css');
PagoHtml::add_css( JURI::root( true ) . '/components/com_pago/templates/default/css/pago.css' );
...
<div id="pago">
	<div id="pg-account" class="pg-print-orders">
		<?php
		if(count($ordArray) > 0)
		{
			...
			for($j = 0; $j < count($ordArray); $j++)
			{
				$order       = $order  = Pago::get_instance('orders')->get($ordArray[$j]);
				$details = $order['details'];
				$addresses=Pago::get_instance('users')->get($details->user_id);
				$orderItems = $order['items'];
				?>
				<div id="pg-account-order-details">
					<span class="pg-account-order-label"><b><?php echo JText::_('PAGO_ACCOUNT_ORDER_NUMBER'); ?>:</b></span> <span class="pg-account-order-number"><?php echo $details->order_id; ?></span><br />
					<span class="pg-account-order-label"><b><?php echo JText::_('PAGO_ACCOUNT_ORDER_DATE'); ?>:</b></span> <span class="pg-account-order-date"><?php echo $details->cdate; ?></span><div><br/></div>
				...
				<div class="cl-b"></div>
				<div style="page-break-after:always;page-break-inside: avoid; page-break-before: always; "></div>
				<hr />
		<?php
