...
class PagoViewTax extends JViewLegacy
{
	protected $items;
	...
	public function display( $tpl = null )
	{
		...
		$this->taxClasses = $this->get('Taxclasses');
		...
		// $this->addToolbar();
		$this->taxcid = JFactory::getApplication()->input->getInt('taxcid');
	...
	function display_form()
	{
		...
		// Toolbar
		$title = JText::_('PAGO_TAX');
		$desc = JText::_('PAGO_TAX_DESC') . $item->pgtax_rate_name;
		...
		$params = new PagoParameter($item_bind,  $cmp_path . 'views/tax/params.xml');
		...
		if ( !$base_params = $params->render('params', 'base', ''))
			$base_params = JText::_('PAGO_TAX_TEMPLATE_ERROR_GENERAL_PARAMETERS');
	...
	protected function parse_items( $items )
	{
		if ( is_array($items) )
		{
			foreach ( $items as $k => $item )
			{
				if ($this->taxcid)
				{
					if ( $item->pgtax_class_id != $this->taxcid )
					{
						unset($items[$k]);
					}
				}
