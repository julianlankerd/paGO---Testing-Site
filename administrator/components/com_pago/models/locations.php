...
/**
 * Methods supporting a list of locations.
 *
 * @package		paGO Commerce
 * @subpackage	com_pago
 * @since		1.6
 */
class PagoModelLocations extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		...
		if (empty($config['filter_fields'])) {
			$config['filter_fields'] = array(
				...
				'country_3_code', 'item.country_3_code',
				'country_2_code', 'item.country_2_code',
	...
	protected function getListQuery()
	{
		...
		$query->from( '`#__pago_country` AS item' );
		...
		if (!empty($search)) {
			$search = $db->Quote('%'.$db->escape($search, true).'%');
				$query->where('(item.country_name LIKE '.$search.'
									OR item.country_id = '.$db->Quote( $this->getState('filter.search') ).')
				');
		}
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Locations', $prefix = 'PagoTable', $config = array())
	{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		parent::populateState('country_id', 'desc');
	}
}