 ...
 * @since		1.6
 */
class PagoModelPlugins extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		if ($folder = $this->getState('filter.folder')) {
			...
			//little hack to show joomla plugin captcha group with security group
			if($folder == 'pago_security')
				$query->where('a.folder = '.$db->quote($folder) . ' OR a.folder = "captcha"');
		}
